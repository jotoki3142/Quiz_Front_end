...
const QuestionModal = ({
    ...
    // Initialize state from props. changes to props will NOT update state unless key changes or we use an effect.
    // Since we unmount when !isOpen, this works for fresh opens.
    // If editingQuestion changes while open, we would need an effect, but that shouldn't happen here.

    // Default initial values
    const initialTitle = editingQuestion?.title || "";
    const initialType = editingQuestion?.type || "";
    const initialDifficulty = editingQuestion?.difficulty || "";
    const initialCategoryId = editingQuestion ? String(editingQuestion.categoryId) : "";
    const initialVisibility = editingQuestion?.visibility || "PRIVATE";
    const initialAnswers = editingQuestion?.answers || [{ text: "", correct: false }, { text: "", correct: false }];
    ...
    // Answer logic
    const handleAnswerChange = (idx: number, field: keyof Answer, value: string | boolean) => {
        ...
        if (field === 'text') {
            newAnswers[idx].text = value as string;
        } else if (field === 'correct') {
            newAnswers[idx].correct = value as boolean;
        }
    ...
    const handleTypeChange = (newType: string) => {
        setType(newType);
        if (!editingQuestion && newType === 'TRUE_FALSE') {
            ...
            <div className="bg-white rounded-3xl w-full max-w-3xl shadow-2xl animate-in fade-in zoom-in duration-200 max-h-[90vh] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="px-8 py-5 bg-zinc-50 border-t border-zinc-100 flex justify-between items-center text-sm">
                    <div className="flex items-center gap-2 text-zinc-500">
                        ...
                        <span>Người tạo: <span className="font-bold text-zinc-700">{question.createdByName || question.createdBy || "Admin"}</span></span>

                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase
                            ${(question.createdByRole || "").toUpperCase() === "ADMIN"
                                ? "bg-fuchsia-100 text-fuchsia-600"
                                : "bg-emerald-100 text-emerald-600"
                            }`}>
                            {question.createdByRole === "ROLE_TEACHER" ? "Teacher" : (question.createdByRole || "Admin")}
                        </span>
                    </div>
                    <button
                        onClick={onClose}
...
export default function TeacherQuestionsPage() {
    ...
    // Initial Load - User Profile & Categories
    useEffect(() => {
        (async () => {
            try {
    ...
    const handleSave = async (data: QuestionFormData) => {
        ...
        try {
            ...
            const payload = {
                ...
                correctAnswer: validAnswers.find((a) => a.correct)?.text || ""
            };
    ...
    const canEditOrDelete = (q: Question) => {
        if (!currentUser) return false;

        // Admin permissions via role check (just in case teacher has admin role)
        const isAdmin = currentUser.authorities?.some(auth => auth.authority === 'ROLE_ADMIN');
        if (isAdmin) return true;

        // If created by Admin -> Teacher cannot edit/delete
        if ((q.createdByRole || "").toUpperCase() === "ADMIN") return false;

        // Otherwise check ownership
        return (q.createdBy || "").toLowerCase() === (currentUser.username || "").toLowerCase();
    };
                        ...
                        <table className="w-full text-left text-sm text-zinc-600">
                            <thead className="bg-zinc-50 border-b border-zinc-200">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 font-bold text-zinc-400 uppercase tracking-wider text-xs whitespace-nowrap">Người tạo</th>
                                    ...
                                    <tr>
                                        <td colSpan={8} className="px-6 py-12 text-center">
                                            <div className="flex flex-col items-center justify-center text-zinc-400">
                                                <QuestionMarkCircleIcon className="w-16 h-16 mb-4 stroke-1 opacity-20" />
                                                <p className="text-lg font-semibold text-zinc-600">Không tìm thấy câu hỏi</p>
                                                <p className="text-sm">Hãy thử thay đổi bộ lọc hoặc thêm câu hỏi mới.</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                    questions.map((q, idx) => (
                                        <tr key={q.id} className="group hover:bg-violet-50/40 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className="font-bold text-zinc-800 text-base group-hover:text-violet-700 transition-colors line-clamp-2" title={q.title}>
                                                    {q.title}
                                                </span>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap max-w-[150px] truncate" title={q.categoryName}>
                                                <span className="px-2 py-1 rounded bg-zinc-100 font-medium text-xs text-zinc-600">
                                                    {q.categoryName || "—"}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="font-bold text-zinc-700">{q.createdByName || q.createdBy || "Admin"}</span>
                                                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase
                                                        ${(q.createdByRole || "").toUpperCase() === "ADMIN"
                                                            ? "bg-fuchsia-100 text-fuchsia-600"
                                                            : "bg-emerald-100 text-emerald-600"
                                                        }`}>
                                                        {q.createdByRole === "ROLE_TEACHER" ? "Teacher" : (q.createdByRole || "Admin")}
                                                    </span>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-right whitespace-nowrap">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    {/* ONLY Show Edit/Delete if allowed */}
                                                    {canEditOrDelete(q) && (
                                                        <>
                                                            <button
                                                                onClick={() => { setEditingQuestion(q); setIsEditModalOpen(true); }}
