    ...
    <div className="min-h-screen">
      <div className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
              <div>
                ...
                <p className="text-violet-100 mt-2 text-lg opacity-90 max-w-2xl">
                  Quản lý các bài thi, câu hỏi và theo dõi tiến độ học tập của học viên.
                </p>
              </div>

              {/* Navigation Tabs */}
              <div className="flex p-1 bg-white/20 backdrop-blur-md rounded-xl border border-white/20">
                <button onClick={() => router.push("/teacher/list-exam")} className="px-4 py-2 rounded-lg bg-white text-[#A53AEC] font-bold shadow-sm transition-all text-sm">
                  Bài thi
                </button>
                <button onClick={() => router.push("/teacher/history-exam")} className="px-4 py-2 rounded-lg text-white hover:bg-white/10 transition-all text-sm font-medium">
                  Lịch sử Offline
                </button>
                <button onClick={() => router.push("/teacher/history-exam-online")} className="px-4 py-2 rounded-lg text-white hover:bg-white/10 transition-all text-sm font-medium">
                  Lịch sử Online
                </button>
              </div>
            </div>
        ...
        <div className="bg-white rounded-3xl border border-zinc-100 shadow-sm min-h-[500px] p-6 space-y-10">
          ...
          <section>
            <h2 className="text-lg font-bold text-zinc-900 flex items-center gap-2 mb-4">
              <span className="w-2 h-8 rounded-full bg-[#A53AEC]"></span>
              Danh sách bài thi
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {readyExams.map((exam) => (
                  <ExamCardOffline
                    ...
                    setOpenMenu={setOpenMenu}
                    onDelete={confirmDelete}
                    onShare={(link: string) => { setShareLink(link); setOpenShare(true); }}
                    router={router}
                    isPublished
                  />
                ))}
          ...
          <div className="bg-white rounded-3xl p-6 w-[480px] shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            {activeTab === "link" ? (
              <div className="space-y-4">
                ...
                <button onClick={() => { navigator.clipboard.writeText(shareLink); toastSuccess("Đã sao chép!"); }} className="w-full py-3 bg-[#A53AEC] hover:bg-purple-700 text-white font-bold rounded-xl transition-all active:scale-95 shadow-lg shadow-purple-200">
                  Sao chép liên kết
                </button>
              </div>
            ) : (
              <div className="flex flex-col items-center py-4">
                ...
                <button className="w-full py-3 bg-[#A53AEC] hover:bg-purple-700 text-white font-bold rounded-xl transition-all active:scale-95 shadow-lg shadow-purple-200">
                  Tải xuống QR
                </button>
              </div>
            )}
...
const ExamCardOffline = ({ exam, openMenu, setOpenMenu, onDelete, onShare, router, isPublished, isFinished }: any) => {
  return (
    <div className={`bg-white rounded-2xl border border-zinc-200 p-5 shadow-sm hover:shadow-md transition-all group relative flex flex-col justify-between h-full ${isPublished ? 'hover:border-purple-300' : 'hover:border-amber-300'}`}>
      <div className="mb-4">
        <div className="flex justify-between items-start mb-2">
          ...
          {openMenu === exam.examId && (
            <div className="absolute right-4 top-12 bg-white shadow-xl ring-1 ring-black/5 rounded-xl py-2 w-48 z-20 animate-in fade-in zoom-in-95 origin-top-right">
              <button onClick={() => router.push(`/teacher/detail-exam/${exam.examId}`)} className="flex w-full items-center px-4 py-2 text-sm text-zinc-700 hover:bg-zinc-50"><EyeIcon className="w-4 h-4 mr-2" /> Chi tiết</button>
              <button onClick={() => router.push(`/teacher/update-exam/${exam.examId}`)} className="flex w-full items-center px-4 py-2 text-sm text-zinc-700 hover:bg-zinc-50"><PencilSquareIcon className="w-4 h-4 mr-2" /> Cập nhật</button>
              {isPublished && onShare && (
                <button onClick={() => { onShare(`${window.location.origin}/teacher/exam/${exam.examId}`); setOpenMenu(null); }} className="flex w-full items-center px-4 py-2 text-sm text-zinc-700 hover:bg-zinc-50"><ShareIcon className="w-4 h-4 mr-2" /> Chia sẻ</button>
              )}
      ...
      {!isPublished && !isFinished && (
        <button onClick={() => router.push(`/teacher/update-exam/${exam.examId}`)} className="w-full mt-4 py-2 rounded-xl bg-amber-100 text-amber-700 font-bold text-sm hover:bg-amber-200 transition-colors">
          Tiếp tục soạn
        </button>
      )}
    </div>
  ...
  return (
    <div className={`bg-white rounded-2xl border-2 p-5 shadow-sm hover:shadow-lg transition-all relative flex flex-col justify-between h-full ${exam.status === 'IN_PROGRESS' ? 'border-green-400 shadow-green-100' : (exam.status === 'WAITING' ? 'border-sky-400 shadow-sky-100' : 'border-zinc-100')}`}>
      <div className="mb-4">
        <div className="flex justify-between items-start mb-2">
          ...
          {openMenu === `online-${exam.id}` && (
            <div className="absolute right-4 top-12 bg-white shadow-xl ring-1 ring-black/5 rounded-xl py-2 w-48 z-20 animate-in fade-in zoom-in-95">
              <button onClick={() => router.push(`/teacher/exam-online/edit/${exam.id}`)} className="flex w-full items-center px-4 py-2 text-sm text-zinc-700 hover:bg-zinc-50"><PencilSquareIcon className="w-4 h-4 mr-2" /> Chỉnh sửa</button>
              <div className="w-full h-px bg-zinc-100 my-1"></div>
        ...
        <p className="text-xs text-zinc-500 font-mono bg-zinc-100 inline-block px-1.5 py-0.5 rounded">CODE: <span className="text-[#A53AEC] font-bold">{exam.accessCode}</span></p>
      </div>
      ...
      {!isFinished && (
        <div className="mt-auto flex flex-col gap-2">
          {exam.status === 'DRAFT' && (
            <>
              <button onClick={() => router.push(`/teacher/exam-online/edit/${exam.id}`)} className="w-full py-2.5 rounded-xl bg-zinc-100 text-zinc-700 font-bold text-sm hover:bg-zinc-200 transition-colors">
                {exam.actualQuestionCount > 0 ? "Chỉnh sửa câu hỏi" : "Thêm câu hỏi"}
              </button>
              {exam.actualQuestionCount > 0 && (
                <button onClick={handleStartExam} className="w-full py-2.5 rounded-xl bg-[#A53AEC] text-white font-bold text-sm hover:bg-purple-700 transition-colors shadow-lg shadow-purple-200 flex items-center justify-center gap-2">
          ...
          {exam.status === 'WAITING' && (
            <button onClick={() => router.push(`/teacher/waiting-room/${exam.accessCode}`)} className="w-full py-2.5 rounded-xl bg-sky-500 text-white font-bold text-sm hover:bg-sky-600 transition-colors shadow-lg shadow-sky-200">
              Vào phòng chờ
            </button>
          )}
          {exam.status === 'IN_PROGRESS' && (
            <button onClick={() => router.push(`/teacher/exam-online/${exam.id}/monitor`)} className="w-full py-2.5 rounded-xl bg-green-500 text-white font-bold text-sm hover:bg-green-600 transition-colors shadow-lg shadow-green-200 flex items-center justify-center gap-2">
      ...
      {isFinished && (
        <button onClick={() => router.push(`/teacher/exam-online/${exam.id}/results`)} className="w-full mt-auto py-2.5 rounded-xl bg-zinc-100 text-zinc-600 font-bold text-sm hover:bg-zinc-200 transition-colors">
          Xem kết quả
        </button>
      )}
    </div>
