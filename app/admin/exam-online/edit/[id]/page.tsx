...
export default function AdminEditOnlineExamPage() {
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // 3. Map Questions (Manual & Library)
                // Note: The API structure for online exams separates manual and library questions usually,
                // or returns them in a specific way. We'll map what we have.
                // Assuming `exam.questions` contains library questions and we might have manual ones if stored?
                // Or if the backend returns all as questions.
                // Based on previous file, it seemed manual questions were stored separately in `manualQuestions`.
                // However, usually we might want to unify them in the UI.

                // Let's assume we fetch all questions related to this exam.
                // If the API `/online-exams/${id}` returns everything, we use it.

                const mappedQuestions: Question[] = [];

                // Standardize Manual Questions
                // Use a check if `manualQuestions` exists
                if (exam.manualQuestions && Array.isArray(exam.manualQuestions)) {
                    // mappedQuestions.push(...exam.manualQuestions.map((q: any) => ({...q, isReadOnly: false})));
                    // Actually, usually manual questions are just questions created with createdBy=TEACHER/Admin
                    // Let's retry fetching logic.
                }

                // Let's rely on what the API gives. If it gives a list of question IDs or objects.
                // Since this is a redesign, we will trust the previous implementation's data shape slightly 
                // but we need to load ALL questions.

                // If the edit endpoint doesn't return full question details, we might need another call.
                // But typically `get/{id}` returns details.

                // Let's assume `exam.questions` has the Library questions (readOnly)
                // And we have no clear way to distinguish manual unless we check `createdBy` or if they were passed separately.
                // We'll treat all questions as "Library/ReadOnly" if they exist, EXCEPT if we can determine they are unique to this exam.
                // BUT, to keep it simple and consistent:
                // We will load ALL questions currently in the exam into the `questions` array.
                // Users can remove them and add new ones.
                // Manual creation adds new editable items.

                // Fetch current questions detail if not in exam object
                // If exam.questionIds is present but not full objects?
                // The previous Code used `fetchApi('/online-exams/${id}')`.

                // Let's look at how we process them.
                // We will assume `exam.examQuestions` or `exam.questions` holds the data.
                ...
                if (exam.questions) {
                    combinedQuestions = exam.questions.map((q: any) => ({
                        ...
                        isReadOnly: true, // Default to readOnly for existing questions to be safe, unless we want to edit specific ones
                        // Ideally, we only allow editing "Manual" questions which might be identified by createdBy
                        createdBy: q.createdBy,
                        visibility: q.visibility
                    }));
                }

                // If `manualQuestions` was a thing in the form logic only, we must see if the backend persists them.
                // Backend likely persists them as real questions in `question` table.

                // So all questions are just questions.
    ...
    const handleSubmit = async (values: ExamFormValues) => {
        try {
            ...
            // 2. Update Exam
            // Only update fields. The questions are updated via a separate call usually, or grouped.
            // Based on previous file: `/online-exams/${examId}/update` for metadata
            // And `/online-exams/${examId}/questions` for linking questions.
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-20">
            {/* Header / Hero */}
            <div className="relative bg-gradient-to-r from-violet-600 to-fuchsia-600 pb-24 pt-12 shadow-xl">
                        ...
                        <Form className="space-y-8 max-w-5xl mx-auto">
                                    ...
                                    {({ push, remove }) => (
                                        <div className="space-y-6">
                                                    ...
                                                    <div className="flex justify-between items-start mb-6">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div className="flex flex-col">
                                                                ...
                                                                {q.isReadOnly && (
                                                                    <span className="text-xs text-gray-500">ID: {q.id} • {q.difficulty}</span>
                                                                )}
                                                            </div>
                                                        </div>
                                                        <button
                                                            ...
                                                            <div className="space-y-3 pl-4 border-l-2 border-gray-100">
                                                                    ...
                                                                    <div key={aIndex} className="flex items-center gap-3 group/answer">
                                                                        <div className="relative flex items-center justify-center">
                                                                            <Field
                                                                                ...
                                                                                className="peer w-5 h-5 cursor-pointer accent-violet-600"
                                                                            />
                                                                        </div>
                                                                        ...
                                                                        <Field
                                                                            ...
                                                                            className={`flex-1 px-3 py-2 rounded-lg outline-none transition-all ${a.isCorrect ? 'bg-green-50 text-green-700 font-medium border-green-200 border' : 'bg-gray-50 border border-transparent'
                                                                                }`}
                                                                        />
                                                                        ...
                                                                        {!q.isReadOnly && q.type !== "TRUE_FALSE" && (
                                                                            <button
                                                                                ...
                                                                                className="opacity-0 group-hover/answer:opacity-100 text-gray-400 hover:text-red-500 transition-all p-1"
                                                                            >
                                                                ...
                                                                {!q.isReadOnly && q.type !== "TRUE_FALSE" && (
                                                                    <button
                                                                        ...
                                                                        className="text-sm text-violet-600 hover:text-violet-700 font-medium flex items-center gap-1 mt-2 px-2 py-1 rounded-md hover:bg-violet-50 transition-colors w-fit"
                                                                    >
                                            ...
                                            <button
                                                ...
                                                className="w-full py-4 border-2 border-dashed border-gray-300 rounded-2xl text-gray-500 hover:border-violet-400 hover:text-violet-600 hover:bg-violet-50 transition-all flex flex-col items-center justify-center gap-2"
                                            >
                                                <PlusCircleIcon className="w-8 h-8 opacity-50" />
                                                <span className="font-medium">Tạo câu hỏi thủ công</span>
                                            </button>
                                        </div>
                                    )}
                                </FieldArray>
                            ...
                            <div className="sticky bottom-4 z-30 bg-white/80 backdrop-blur-md border border-white/20 shadow-xl rounded-2xl p-4 flex justify-between items-center max-w-5xl mx-auto">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <button
                                        type="submit"
                                        className="px-8 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white font-bold shadow-lg shadow-violet-200 hover:shadow-xl hover:scale-[1.02] transition-all"
                                    >
                                        ...
                                        <div className="p-6 bg-white border-t border-gray-100 flex justify-between items-center shrink-0">
                                            ...
                                            <div className="flex gap-4">
                                                ...
                                                <button
                                                    ...
                                                    className="px-8 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white font-bold shadow-lg shadow-violet-200 hover:shadow-xl hover:scale-105 disabled:opacity-50 disabled:hover:scale-100 transition-all"
                                                >
                                                    Thêm vào bài thi
                                                </button>
                                            </div>
