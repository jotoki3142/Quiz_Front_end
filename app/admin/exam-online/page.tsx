...
export default function CreateOnlineExamPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/admin/exam-online/edit/${response.id}`);
    } catch (error: any) {
        ...
        <main className="px-4 md:px-0">
          <section className="bg-white rounded-3xl border border-zinc-100 shadow-sm p-8 max-w-5xl mx-auto">
            <div className="flex gap-4 p-1 bg-zinc-100/50 rounded-xl mb-8 w-fit">
              <a href="/admin/exam-offline" className="flex items-center gap-2 px-6 py-2.5 text-zinc-500 font-medium hover:text-violet-600 hover:bg-white/50 rounded-lg transition-all">
                <BookOpenIcon className="w-5 h-5" />
                Bài thi Offline
              </a>
              <a href="/admin/exam-online" className="flex items-center gap-2 px-6 py-2.5 bg-white text-violet-600 font-bold rounded-lg shadow-sm border border-zinc-200/50 transition-all">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-6 border-t border-zinc-100 mt-2">
                ...
                <div className="flex items-center gap-4 w-full md:w-auto">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-1 md:flex-none px-8 py-3 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white font-bold rounded-xl shadow-lg shadow-violet-200 hover:shadow-xl hover:scale-[1.02] transition-all flex items-center justify-center gap-2 min-w-[160px]"
                  >
                    {loading ? (
                      "Đang tạo..."
                    ) : (
